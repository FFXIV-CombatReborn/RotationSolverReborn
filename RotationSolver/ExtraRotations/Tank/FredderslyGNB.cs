...
public sealed class FredderslyGNB : GunbreakerRotation
{
	...
	private bool ShouldUseLevel100NoMercy(IAction nextGCD)
	{
		...
		// After the opener, the filler Gnashing sequence should already be in
		// progress; late weave No Mercy when the next GCD is Reign of Beasts.
		return nextGCD.IsTheSameTo(false, (ActionID)ReignOfBeastsPvE.ID);
	}
	...
	protected override bool AttackAbility(IAction nextGCD, out IAction? act)
	{
		...
		{
			if (HasNoMercy || !NoMercyPvE.EnoughLevel || NoMercyPvE.Cooldown.WillHaveOneChargeGCD(1))
			{
				return true;
			}
		}
