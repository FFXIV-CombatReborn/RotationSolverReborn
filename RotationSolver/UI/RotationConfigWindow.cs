...
public partial class RotationConfigWindow : Window
{
    ...
    private static void DrawDutyRotation()
    {
        ...
        ImguiTooltips.HoveredTooltip("The duty rotation you chose. Click to modify.");
    ...
    private void DrawDiagnosticInfoCube()
    {
        ...
        var installedIncompatiblePlugin = CommunityPlugin.IncompatiblePlugins.FirstOrDefault(p => p.IsEnabled)!;
    ...
    private void DrawSideBar()
    {
        ...
        if (child)
        {
            ...
            if (windowWidth > JOB_ICON_WIDTH * Scale)
            {
                DrawDutyRotation();
                if (CheckErrors())
                {
    ...
    private void DrawTabs()
    {
        ...
        {
            var windowWidth = ImGui.GetWindowSize().X;
            var iconSize = Math.Max(Scale * MIN_COLUMN_WIDTH, Math.Min(windowWidth, Scale * JOB_ICON_WIDTH)) * 0.6f;
            ...
            if (IconSet.GetTexture(tab.GetAttribute<TabIconAttribute>()?.Icon ?? 0, out var icon) && windowWidth <= JOB_ICON_WIDTH * Scale)
            {
                ImGuiHelper.DrawItemMiddle(() =>
                {
                    ...
                    if (ImGuiHelper.NoPaddingNoColorImageButton(icon.ImGuiHandle, Vector2.One * iconSize, tab.ToString()))
                    {
                        _activeTab = tab;
                        _searchResults = [];
                    }
                    ImGuiHelper.DrawActionOverlay(cursor, iconSize, _activeTab == tab ? 1 : 0);
                }, Math.Max(Scale * MIN_COLUMN_WIDTH, windowWidth), iconSize);

                var desc = tab.ToString();
                var addition = tab.GetDescription();
                if (!string.IsNullOrEmpty(addition)) desc += "\n \n" + addition;
                ImguiTooltips.HoveredTooltip(desc);
            }
            else
            {
                if (ImGui.Selectable(tab.ToString(), _activeTab == tab, ImGuiSelectableFlags.None, new Vector2(0, 20)))
                {
                    _activeTab = tab;
                    _searchResults = [];
                }
            ...
            if (tab == RotationConfigWindowTab.Debug)
            {
                ImGui.Separator();
            }
        }
    ...
    {
        var size = MathF.Max(MathF.Min(windowWidth, Scale * 128), Scale * MIN_COLUMN_WIDTH);
        ...
        if (IconSet.GetTexture((uint)0, out var overlay))
        {
            ...
            ImGuiHelper.DrawItemMiddle(() =>
            {
                ...
                ImguiTooltips.HoveredTooltip("Analyzes PvE combat information in every frame and calculates an effectiveaction.");
        ...
        Player.Job == Job.MIN || Player.Job == Job.FSH || Player.Job == Job.BTN))
        {
            ...
            var text = "No rotations loaded! Please check the rotations tab!";
            if (text == null)
            {
                Svc.Log.Error("The 'No rotations loaded' text is null.");
                return;
            }
        ...
        if (rotation != null)
        {
            ...
            var iconSize = Math.Max(Scale * MIN_COLUMN_WIDTH, Math.Min(windowWidth, Scale * JOB_ICON_WIDTH));
            ...
            if (Scale * JOB_ICON_WIDTH < windowWidth)
            {
    ...
    private void DrawRotationIcon(ICustomRotation rotation, float iconSize)
    {
        ...
        // Show tooltip if the tab is hovered
        if (ImGui.IsItemHovered())
        {
        ...
        if (child)
        {
            ...
            if (_searchResults != null && _searchResults.Length != 0)
            {
                ...
                using (var font = ImRaii.PushFont(FontManager.GetFont(18)))
                {
                    ...
                    ImGui.TextWrapped("Search Result");
                }
...
{
    { 100_000, "Well, you must be a lazy player!" },
    { 500_000, "You're tiring RSR out - give it a break!" },
};
    ...
    private static void DrawAbout()
    {
        ...
        {
            using var color = ImRaii.PushColor(ImGuiCol.Text, ImGui.ColorConvertFloat4ToU32(ImGuiColors.DalamudYellow));
            ImGui.TextWrapped("Analyzes PvE combat information in every frame and calculates an effective action.");
        }

        ImGui.Spacing();

        // Draw the description
        ImGui.TextWrapped("This includes almost all information available in one combat frame, including the status of all party members, hostile target statuses, skill cooldowns, MP and HP of characters, character locations, hostile target casting status, combo state, combat duration, player level, etc.\n\nIt will then highlight the best action on the hotbar, or help you click it.");
        ...
        using (var color = ImRaii.PushColor(ImGuiCol.Text, ImGui.ColorConvertFloat4ToU32(ImGuiColors.DalamudOrange)))
        {
            ImGui.TextWrapped("This is designed for GENERAL COMBAT, not for Savage or Ultimate content. Use it carefully.");
        }
        ...
        if (clickingCount > 0)
        {
            ...
            var countStr = string.Format("RSR has helped you by clicking actions {0:N0} times.", clickingCount);
            ImGuiHelper.DrawItemMiddle(() =>
            {
    ...
    private static void DrawAutoStatusOrderConfig()
    {
        ...
        int tabCount = autoStatusOrder.Count;

        for (int i = 0; i < tabCount; i++)
        {
            var tab = autoStatusOrder[i];
            var tabName = Enum.GetName(typeof(AutoStatus), tab) ?? tab.ToString(); // Retrieve the status name by its enum value
            ...
            // Draw down button
            if (ImGuiEx.IconButton(FontAwesomeIcon.ArrowDown, $"##Down{i}") && i < tabCount - 1)
            {
    ...
    {
        { () => "Macros", DrawAboutMacros },
        { () => "Compatibility", DrawAboutCompatibility },
        { () => "Links", DrawAboutLinks },
    });

    private static void DrawAboutMacros()
    {
        // Adjust tab spacing for better layout
        ...
        foreach (StateCommandType stateCommand in Enum.GetValues<StateCommandType>())
        {
            DisplayCommandHelp(stateCommand);
        }

        // Display command help for other commands
        foreach (OtherCommandType otherCommand in Enum.GetValues<OtherCommandType>())
        {
            DisplayCommandHelp(otherCommand);
        }

        ImGui.NewLine();

        // Display command help for special commands
        foreach (SpecialCommandType specialCommand in Enum.GetValues<SpecialCommandType>())
        {
            DisplayCommandHelp(specialCommand);
        }
    }
    ...
    private static void DrawAboutCompatibility()
    {
        ...
        ImGui.TextWrapped("Rotation Solver helps you choose targets and click actions. Any plugin that changes these will affect its decisions.\n\nHere is a list of known incompatible plugins:");
        ...
        if (table)
        {
            ...
            foreach (var plugin in CommunityPlugin.IncompatiblePlugins)
            {
                ...
                var icon = string.IsNullOrEmpty(plugin.Icon)
    ...
    private void DrawAutoduty()
    {
        ...
        var pluginsToCheck = new List<CommunityPlugin>
    {
        new CommunityPlugin { Name = "AutoDuty", Url = "https://puni.sh/api/repository/herc" },
        new CommunityPlugin { Name = "vnavmesh", Url = "https://puni.sh/api/repository/veyn" },
        new CommunityPlugin { Name = "BossModReborn", Url = "https://raw.githubusercontent.com/FFXIV-CombatReborn/CombatRebornRepo/main/pluginmaster.json" },
        new CommunityPlugin { Name = "Boss Mod", Url = "https://puni.sh/api/repository/veyn" },
        new CommunityPlugin { Name = "Avarice", Url = "https://love.puni.sh/ment.json" },
        new CommunityPlugin { Name = "Deliveroo", Url = "https://plugins.carvel.li/" },
        new CommunityPlugin { Name = "AutoRetainer", Url = "https://love.puni.sh/ment.json" },
        new CommunityPlugin { Name = "SkipCutscene", Url = "https://raw.githubusercontent.com/KangasZ/DalamudPluginRepository/main/plugin_repository.json" },
        new CommunityPlugin { Name = "AntiAfkKick", Url = "https://raw.githubusercontent.com/NightmareXIV/MyDalamudPlugins/main/pluginmaster.json" },
    ...
    {
        { () => "Description", DrawRotationDescription },

        { GetRotationStatusHead,  DrawRotationStatus },

        { () => "Configuration", DrawRotationConfiguration },

        { () => "Information", DrawRotationInformation },
    });
    ...
    private static void DrawRotationDescription()
    {
            ...
            if (table)
            {
                foreach (var a in RotationDescAttribute.Merge(attrs))
                {
                    ...
                    foreach (var tab in allActions)
                    {
                        ...
                        if (tab.GetTexture(out var texture))
                        {
                            ...
                            ImguiTooltips.HoveredTooltip(tab.Name);
                        }
                        notStart = true;
                    }
                }
        ...
        {
            DrawLinkDescription(link.LinkDescription, windowWidth, true);
        }
    }

    internal static void DrawLinkDescription(LinkDescription link, float windowWidth, bool drawQuestion)
    {
        var hasTexture = IconSet.GetTexture(link.Url, out var texture);

        if (hasTexture && ImGuiHelper.TextureButton(texture, windowWidth, windowWidth))
        {
    ...
    private static void DrawRotationInformation()
    {
        ...
        {
            ImGui.NewLine();
            if (IconSet.GetTexture("https://www.gstatic.com/youtube/img/branding/youtubelogo/svg/youtubelogo.svg", out var icon) && ImGuiHelper.TextureButton(icon, windowWidth, 250 * Scale, "Youtube Link"))
            {
    ...
    {
        ImGui.TextWrapped("Used to customize when RSR uses specific actions automatically. Click on an action's icon in the left list. Below, you may set the conditions for when that specific action is used. Each action can have different conditions to override the default rotation behavior.");
        ...
        if (table)
        {
            ...
            if (_actionsList != null)
            {
                ...
                if (DataCenter.CurrentRotation != null && RotationUpdater.AllGroupedActions != null)
                {
                    ...
                    foreach (var pair in RotationUpdater.AllGroupedActions)
                    {
                        ...
                        {
                            var index = 0;
                            foreach (var tab in pair.OrderBy(t => t.ID))
                            {
                                if (!tab.GetTexture(out var icon)) continue;
                                ...
                                ImGui.BeginGroup();
                                var cursor = ImGui.GetCursorPos();
                                if (ImGuiHelper.NoPaddingNoColorImageButton(icon.ImGuiHandle, Vector2.One * size, tab.Name + tab.ID))
                                {
                                    _activeAction = tab;
                                }
                                ImGuiHelper.DrawActionOverlay(cursor, size, _activeAction == tab ? 1 : 0);
                                ...
                                if (IconSet.GetTexture("ui/uld/readycheck_hr1.tex", out var texture))
                                {
                                    ...
                                    ImGui.Image(texture.ImGuiHandle, Vector2.One * size * 0.5f,
                                        new Vector2(tab.IsEnabled ? 0 : 0.5f, 0) + offset,
                                        new Vector2(tab.IsEnabled ? 0.5f : 1, 1) - offset);
                                }
                                ImGui.EndGroup();

                                string key = $"Action Macro Usage {tab.Name} {tab.ID}";
                                var cmd = ToCommandStr(OtherCommandType.DoActions, $"{tab}-{5}");
                                ImGuiHelper.DrawHotKeysPopup(key, cmd);
                                ImGuiHelper.ExecuteHotKeysPopup(key, cmd, tab.Name, false);
                            }
                        });
                    }
                }
            ...
            ImGui.TextWrapped("Forced Conditions have higher priority. If Forced Conditions are met, Disabled Conditions will be ignored.");
            _sequencerList?.Draw();
        }
        ...
        static void DrawConfigsOfAction()
        {
            ...
            if (ImGui.Checkbox($"{"Show on CD window"}##{_activeAction.Name}InCooldown", ref enable))
            {
            ...
            static void DrawConfigsOfBaseAction(IBaseAction a)
            {
                ...
                if (ImGui.DragFloat($"{"Time-to-kill threshold required for this action to be used"}##{a}",
                ...
                if (a.Setting.StatusProvide != null || a.Setting.TargetStatusProvide != null)
                {
                    ...
                    if (ImGui.Checkbox($"{"Should this action check status effects"}##{a}", ref shouldStatus))
                    {
                    ...
                    if (shouldStatus)
                    {
                        ...
                        ImGui.SetNextItemWidth(Scale * 150);
                        if (ImGui.DragInt($"{"Number of GCDs before the DoT is reapplied"}##{a}",
                ...
                if (!a.TargetInfo.IsSingleTarget)
                {
                    ...
                    ImGui.SetNextItemWidth(Scale * 150);
                    if (ImGui.DragInt($"{"Number of targets needed to use this action"}##{a}",
                ...
                ImGui.SetNextItemWidth(Scale * 150);
                if (ImGui.DragFloat($"{"HP ratio for automatic healing"}##{a}",
        ...
        static void DrawActionDebug()
        {
            ...
            else if (_activeAction is IBaseItem tab)
            {
                try
                {
                    ...
                    {
                        ImGui.Text("Status: " + ActionManager.Instance()->GetActionStatus(ActionType.Item, tab.ID).ToString());
                        ImGui.Text("Status HQ: " + ActionManager.Instance()->GetActionStatus(ActionType.Item, tab.ID + 1000000).ToString());
                        var remain = ActionManager.Instance()->GetRecastTime(ActionType.Item, tab.ID) - ActionManager.Instance()->GetRecastTimeElapsed(ActionType.Item, tab.ID);
                        ImGui.Text("remain: " + remain.ToString());
                    }

                    ImGui.Text("CanUse: " + tab.CanUse(out _, true).ToString());

                    if (tab is HpPotionItem healPotionItem)
                    {
    ...
    {
        { () => "Force Usage Condition", () =>
        {
            ImGui.TextWrapped("Conditions for forced automatic use of action");
        ...
        { () => "Disable Usage Condition", () =>
        {
            ImGui.TextWrapped("Conditions that disable automatic use of an action");
    ...
    private static void DrawRotations()
    {
        ...
        var text = "Custom rotations are like plugins and have full access to the game and your computer";
        var textWidth = ImGuiHelpers.GetButtonSize(text).X;
        ImGuiHelper.DrawItemMiddle(() =>
        {
            ImGui.TextColored(ImGuiColors.DalamudYellow, text);
        }, width, textWidth);
        text = "Please only load rotations from sources that you trust";
    ...
    {
        { () => "Loaded Rotations", DrawRotationsLoaded},
    });
    ...
    private static void DrawGitHubBadge(string userName, string repository, string id = "", string link = "", bool center = false)
    {
        ...
        if (IconSet.GetTexture($"https://github-readme-stats.vercel.app/api/pin/?username={userName}&repo={repository}&theme=dark", out var icon)
            && (center ? ImGuiHelper.TextureButton(icon, windowWidth, icon.Width, id)
        ...
        if (center)
        {
            ...
            var ratio = MathF.Min(1, windowWidth / width);
            ImGuiHelper.DrawItemMiddle(() =>
            {
    ...
    private static void DrawRotationsLibraries()
    {
        ...
        for (int i = 0; i < Service.Config.RotationLibs.Length; i++)
        {
            ...
            ImGui.InputTextWithHint($"##Rotation Solver OtherLib{i}", "The folder containing the rotation library or the download URL for the rotation library", ref Service.Config.RotationLibs[i], 1024);
            ImGui.SameLine();
    ...
    {
        ImGui.TextWrapped("In this window, you can set parameters that can be customized using lists.");
        _idsHeader?.Draw();
    }

    private static readonly CollapsingHeaderGroup _idsHeader = new(new()
    {
        { () => "Statuses", DrawListStatuses},
        { () => Service.Config.UseDefenseAbility ? "Actions" : string.Empty, DrawListActions},
        { () => "Territories", DrawListTerritories},
    });
    ...
    private static void DrawListStatuses()
    {
        ...
        if (table)
        {
            ...
            ImGui.TableHeader("Invulnerability");
            ...
            ImGui.TableHeader("Dispellable debuffs");
    ...
    private static void FromClipBoardButton(HashSet<uint> tabs)
    {
        ...
        if (ImGui.Button("Copy to Clipboard"))
        {
            try
            {
                ImGui.SetClipboardText(JsonConvert.SerializeObject(tabs));
            }
            catch (Exception ex)
            {
    ...
    private static void DrawStatusList(string name, HashSet<uint> statuses, Status[] allStatus)
    {
        ...
        foreach (var statusId in statuses)
        {
            ...
            ImGuiHelper.DrawHotKeysPopup(key, string.Empty, ("Remove", Delete, ["Delete"]));
    ...
    internal static void StatusPopUp(string popupId, Status[] allStatus, ref string searching, Action<Status> clicked, uint notLoadId = 10100, float size = 32)
    {
        ...
        if (popup)
        {
            ...
            ImGui.InputTextWithHint("##Searching the status", "Status name or ID", ref searching, InputTextLength);
    ...
    private static void DrawListActions()
    {
        ...
        if (table)
        {
            ...
            ImGui.TableHeader("Gaze/Stop");

            ImGui.TableNextRow();

            ImGui.TableNextColumn();
            ImGui.TextWrapped("Use tank personal damage mitigation abilities if the target is casting any of these actions");
            ...
            ImGui.TextWrapped("Use AoE damage mitigation abilities if the target is casting any of these actions");
            ...
            ImGui.TextWrapped("Use knockback prevention abilities if the target is casting any of these actions");
            ...
            ImGui.TextWrapped("Stop casting or taking actions if the enemy is casting this ability");
    ...
    private static void DrawActionsList(string name, HashSet<uint> actions)
    {
        ...
        if (ImGui.Button($"{"Add Action"}##{name}"))
        {
            if (!ImGui.IsPopupOpen(popupId)) ImGui.OpenPopup(popupId);
        }
            ...
            .OrderByDescending(s => SearchableCollection.Similarity($"{s!.Name} {s.RowId}", _actionSearching)))
        {
            ...
            ImGuiHelper.DrawHotKeysPopup(key, string.Empty, ("Remove", Reset, ["Delete"]));
    ...
    private static void DrawListTerritories()
    {
        ...
        if (table)
        {
            ...
            ImGui.TableNextColumn();
            ImGui.TableHeader("Don't target");

            ImGui.TableNextColumn();
            ImGui.TableHeader("Don't provoke");

            ImGui.TableNextColumn();
            ImGui.TableHeader("Beneficial AoE locations");

            ImGui.TableNextRow();

            ImGui.TableNextColumn();
            ImGui.TextWrapped("Enemies that will never be targeted");
            ...
            {
                ImGui.SetNextItemWidth(width);
                if (ImGui.InputTextWithHint($"##Rotation Solver Territory Target Name {i}", "The name of the enemy that you don't want to target", ref libs[i], 1024))
                {
                    OtherConfiguration.NoHostileNames[territoryId] = libs;
            ...
            ImGui.TableNextColumn();
            ImGui.TextWrapped("Enemies that will never be provoked");
            ...
            {
                ImGui.SetNextItemWidth(width);
                if (ImGui.InputTextWithHint($"##Rotation Solver Reborn Territory Provoke Name {i}", "The name of the enemy that you don't want to provoke", ref libs[i], 1024))
                {
                    OtherConfiguration.NoProvokeNames[territoryId] = libs;
            ...
            if (ImGui.Button("Add beneficial AoE location") && Player.Available) unsafe
                {
    ...
    internal static void DrawContentFinder(uint imageId)
    {
        ...
        {
            var windowWidth = ImGui.GetWindowWidth();
            var size = new Vector2(badgeTexture.Width, badgeTexture.Height) * MathF.Min(1, MathF.Min(MaxWidth, windowWidth) / badgeTexture.Width);
...
#pragma warning disable CS0436
    ...
    private static unsafe void DrawStatus()
    {
        ...
        foreach (var tab in DataCenter.AttackedTargets)
        {
            ImGui.Text(tab.id.ToString());
        }

        // VFX info
        //ImGui.Text("VFX Data:");
        //foreach (var tab in DataCenter.VfxDataQueue)
        //{
        //    ImGui.Text(tab.ToString());
        //}
    ...
    private static unsafe void DrawTargetData()
    {
        ...
        foreach (var tab in DataCenter.AllHostileTargets)
        {
            ImGui.Text(tab.Name.ToString());
        }
    }
